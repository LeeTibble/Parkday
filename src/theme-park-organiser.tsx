...
const PRESET_PARKS = [
  {
    ...
    rides: [
      ...
      { name: "The Curse at Alton Manor",          thrill:"🟡", height:"90cm",  rating:4, mustRide:true,  tip:"One of the best dark rides in the UK" },
      ...
      { name: "Nemesis Sub-Terra",                 thrill:"🟡", height:"140cm", rating:3, mustRide:false, tip:"Claustrophobic underground drop ride" },
...
export default function App() {
  ...
  const [workerUrl,    setWorkerUrl]    = useState("https://parkday.ltibble.workers.dev");
  const workerUrlRef = useRef("https://parkday.ltibble.workers.dev");
  ...
  useEffect(() => {
    try {
      ...
      // Only override hardcoded URL if localStorage has a valid non-empty URL
      if (saved && saved.startsWith("http")) { setWorkerUrl(saved); workerUrlRef.current = saved; }
    } catch(e) {}
  ...
  const fetchQueues = useCallback(async (pid, rideList, parkQtId) => {
    ...
    const wUrl = "https://parkday.ltibble.workers.dev";
    if (parkQtId) {
      try {
        ...
        alert("❌ fetchQueues failed: " + e.message + " | wUrl=" + wUrl + " | parkQtId=" + parkQtId);
  ...
  }, [activeParkId, activeRideCount, fetchQueues]);
            ...
            <div style={{ ...GLASS_MID, padding:"10px 16px", display:"flex", alignItems:"center", justifyContent:"space-between", position:"relative", zIndex:3, margin:"0 14px 10px", borderRadius:14 }}>
              <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                ...
                <span style={{ fontSize:13, color:"#EBEBF5", fontWeight:600 }}>
                  {lq?.loading ? "Fetching queue times…"
                    : lq?.lastFetched
                      ? lq.simulated
                        ? "⚠️ Estimated · Park closed or data unavailable"
              ...
              <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                ...
                <button onClick={async ()=>{
                  try {
                    const r = await fetch("https://parkday.ltibble.workers.dev/queues?parkId=" + (park?.queueTimesId||1));
                    const j = await r.json();
                    const count = (j.lands||[]).reduce((a,l)=>a+(l.rides||[]).length,0);
                    alert("Worker OK! " + count + " rides found. First: " + (j.lands?.[0]?.rides?.[0]?.name||"?"));
                  } catch(e) {
                    alert("Worker FAILED: " + e.message);
                  }
                }} style={{ ...GLASS_LIGHT, border:GLASS_LIGHT.border, borderRadius:8, padding:"5px 10px", color:"#FFD93D", fontSize:13, fontWeight:700, cursor:"pointer" }}>
                  Test
                </button>
    ...
    <div>
      ...
      <div style={{ padding:"16px 16px 60px", minHeight:"100vh" }}>
        ...
        {section==="rides" && <>
          ...
          <button onClick={async ()=>{
            try {
              const r = await fetch("https://parkday.ltibble.workers.dev/queues?parkId=1");
              const j = await r.json();
              const count = (j.lands||[]).reduce((a,l)=>a+(l.rides||[]).length,0);
              alert("✅ Worker OK! " + count + " rides. " + (j.lands?.[0]?.rides?.[0]?.name||"?") + " = " + (j.lands?.[0]?.rides?.[0]?.wait_time||"?") + "min");
            } catch(e) {
              alert("❌ Failed: " + e.message);
            }
          }} style={{ width:"100%", background:"rgba(255,200,0,0.15)", border:"1px solid rgba(255,200,0,0.3)", borderRadius:12, padding:"10px", color:"#FFD93D", fontSize:14, fontWeight:700, cursor:"pointer", marginBottom:10 }}>
            🔧 Test Live Queue Connection
          </button>
