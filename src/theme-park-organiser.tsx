...
export default function App() {
  ...
  const fetchQueues = useCallback(async (pid, rideList, parkQtId) => {
    ...
    const isLive = !!wUrl && Object.keys(map||{}).length > 0 && !simulated;
    setLiveQueues(prev => ({ ...prev, [pid]: { data: map, lastFetched: new Date(), loading: false, error: null, simulated: !isLive } }));
  }, [workerUrl]);
