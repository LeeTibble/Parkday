...
function generateAutoPlan(park, pdata, liveQueues) {
  ...
  function getRideWait(ride) {
    ...
    if (liveQueues?.data) {
      ...
      if (live && live.status !== "closed") return live.wait || 10; // Live data — no multiplier
...
export default function App() {
  ...
  const fetchQueues = useCallback(async (pid, rideList, parkQtId) => {
    ...
    // If we got data and have a worker URL, it's live
    const finalSimulated = Object.keys(map||{}).length === 0;
    setLiveQueues(prev => ({ ...prev, [pid]: { data: map, lastFetched: new Date(), loading: false, error: null, simulated: finalSimulated } }));
  }, [workerUrl]);
      ...
      {section==="rides" && rides.length > 0 && (
        <div style={{ ...GLASS_MID, padding:"10px 16px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          <div style={{ display:"flex",alignItems:"center",gap:8 }}>
            ...
            <span style={{ fontSize:13,color:"#EBEBF5",fontWeight:600 }}>
              {liveQueues?.loading ? "Fetching live data…"
